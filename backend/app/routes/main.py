from flask import Blueprint, jsonify, send_from_directory, Flask
import os
...
def get_outfit():
    try:
        ...
        if outfit:
            print(f"Recommended top: {outfit['top']}, bottom: {outfit['bottom']}")
            # 画像パスをフロントエンドが読み取れる形で変更
            outfit['top'] = f"/images/{outfit['top']}"
            outfit['bottom'] = f"/images/{outfit['bottom']}"
            print(f"Modified top path: {outfit['top']}, bottom path: {outfit['bottom']}")
